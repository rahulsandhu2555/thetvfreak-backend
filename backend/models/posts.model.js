...
Post.getAll = (pageNumber, result) => {
    let query = "SELECT posts.*, postmeta.meta_value as featured_image FROM posts " +
        "LEFT JOIN postmeta ON posts.ID = postmeta.post_id " +
        "where posts.post_type = 'post' and postmeta.meta_key = '_thumbnail_id' " +
        "ORDER BY post_date desc " +
        `limit ${pageNumber*dbConfig.listPerPage}, ${dbConfig.listPerPage}`;
    console.log(query)
